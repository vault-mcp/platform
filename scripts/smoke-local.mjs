...
try {
  ...
  assert(health.document_count > 0, "expected synced documents");

  const metadata = await json(`${baseUrl}/.well-known/oauth-protected-resource`);
  assert(metadata.resource === `${baseUrl}/mcp`, "expected protected-resource metadata for /mcp");
  ...
  const fetchedByPath = await mcp(31, "tools/call", {
    ...
    arguments: { path: expectedProjectPath },
  });
