...
import type { IndexPolicy, IndexStats, SyncManifest, VaultDocument, VaultIndex } from "./types.js";
...
export async function buildVaultIndex(options: BuildVaultIndexOptions): Promise<VaultIndex> {
  ...
  const indexPolicy = options.indexPolicy ?? defaultIndexPolicy();
  ...
  for (const filePath of markdownFiles) {
    ...
    const pathOnlyPolicy = evaluateSourcePolicy(relativePath, [], null, indexPolicy);
    ...
    const policy = evaluateSourcePolicy(relativePath, parsed.tags, parsed.status, indexPolicy);

    if (!policy.allowed) {
      if (policy.reviewRequired) {
        recordReview(stats, policy.matchedRule);
        continue;
      }
      ...
      documents.push({
        ...
        metadata: {
          ...
          source_policy: {
            ...
            policy_version: indexPolicy.version,
            index_mode: indexPolicy.mode,
          },
        },
      });
    }
  ...
  const manifest: SyncManifest = {
    ...
    policy_version: indexPolicy.version,
    index_mode: indexPolicy.mode,
    policy_summary: summarizeIndexPolicy(indexPolicy),
  };
...
function recordReview(stats: IndexStats, rule: string): void {
  stats.review_required_markdown = (stats.review_required_markdown ?? 0) + 1;
  stats.reviewed_by_rule![rule] = (stats.reviewed_by_rule![rule] ?? 0) + 1;
}
