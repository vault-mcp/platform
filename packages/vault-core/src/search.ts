...
export function fetchDocumentByPath(documents: VaultDocument[], notePath: string): FetchResponse | null {
  const matches = documents
    .filter((document) => document.metadata.path === notePath)
...
export function recentNotes(documents: VaultDocument[], scope?: string, limit = DEFAULT_LIMIT): { notes: NoteSummary[] } {
  const notes = groupByNote(documents)
    .map((note) => note.summary)
    .filter((note) => !scope || note.path.startsWith(scope))
...
export function activeProjects(documents: VaultDocument[], limit = DEFAULT_LIMIT, cursor?: string): ListNotesResponse {
...
export function getIndexStatus(documents: VaultDocument[], _stats: IndexStats | null, lastIndexedAt: string | null): IndexStatusResponse {
  return {
    indexed_note_count: groupByNote(documents).length,
    indexed_section_count: documents.length,
...
export function debugSearch(documents: VaultDocument[], query: string, scope: string | undefined, lastIndexedAt: string | null): DebugSearchResponse {
  const normalizedQuery = normalize(query);
  const expanded = expandedQuery(normalizedQuery).expandedTerms;
  const resultCount = normalizedQuery ? searchVault(documents, { query, scope, limit: 5 }).results.length : 0;
...
function groupByNote(documents: VaultDocument[]): NoteGroup[] {
  ...
  for (const document of documents) {
    const group = groups.get(document.metadata.path) ?? [];
    group.push(document);
    groups.set(document.metadata.path, group);
  }
